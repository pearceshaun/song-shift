...
- [x] **4.2** Write tests for search and read-only methods in `tests/providers/test_shazam.py`: `TestShazamSearch` class (test_search_track_returns_mapped_results, test_search_track_empty_results, test_search_track_by_isrc_returns_none) and `TestShazamReadOnly` class (test_create_playlist_raises_not_implemented, test_add_tracks_raises_not_implemented). Mock `shazamio.Shazam` for search tests. Run `python -m pytest` -- all pass. Commit and push.
